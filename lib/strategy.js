...
function Strategy(options, verify) {
  ...
  this._userProfileWithEmailURL = options.userProfileWithEmailURL || 'https://api.twitter.com/1.1/account/verify_credentials.json';
  this._skipExtendedUserProfile = (options.skipExtendedUserProfile !== undefined) ? options.skipExtendedUserProfile : false;
  this._includeProfileEmail = (options.includeProfileEmail !== undefined) ? options.includeProfileEmail : false;
}
...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  if (!this._skipExtendedUserProfile) {
    ...
    var profileURL;
    if(this._includeProfileEmail) {
      profileURL = this._userProfileWithEmailURL + '?include_email=true&user_id=' + params.user_id;
    } else {
      profileURL = this._userProfileURL + '?user_id=' + params.user_id;
    }
    
    this._oauth.get(profileURL, token, tokenSecret, function (err, body, res) {
      if (err) {
        if (err.data) {
          try {
